...
- 私有仓库 [HOS-DATA](https://github.com/lxcxjxhx/HOS-DATA) 是外部数据的采集与存储中枢：**data 子分支（文件级隔离，仅数据文件）+ SQLite 存储**，main 分支只放代码。文件管理约定：`data/<数据域>/`（一个数据域一个独立目录，内含 `<域>.db` SQLite 库 + `<域>.json` JSON 快照 + `archive/` 归档，新增域自动建目录/建库）、`data/manifest.json`（数据库台账，自动维护库名/目录/来源/schema 版本/更新时间）；库内表结构统一为 `meta` + `latest` + `history`（数据湖）。cron 每 6 小时采集后提交到 data 子分支，后续更多数据搜集需求在此统一管理（设 `HOS_DB_DIR` 环境变量即启用 SQLite 双写）。
